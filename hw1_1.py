...
def compute_weight(x_matrix, total_days=252, method="Markowitz", starttime=0, endtime=0):
    ...
    if method == "MontoCarlo":
        ...
        bar = tqdm(list(range(int(MONTO_CARLO_TIMES))), bar_format='{l_bar}%s{bar}%s{r_bar}' % (Fore.BLUE, Fore.RESET))
        for _ in bar:
            ...
            sigma_p = np.sqrt(np.matmul(np.matmul(weights_row_vector, cov_matrix_numpy), weights_row_vector.T))[0][0]
...
def get_six_month_map(x_matrix):
    ...
    dfx["month"] = pd.to_datetime(pd.DataFrame(x_matrix.index, columns=['time'])['time'], format='%Y-%m-%d').dt.month
...
def save_weights_markowitz():
    x_matrix_total = process_data_x_matrix(DATAPATH)
    six_map, compare_list = get_six_month_map(x_matrix_total)
    ...
    for k, v in bar:
        ...
        total_days = k - v
        weight = compute_weight(df_weight, total_days)
...
def save_weights_montocarlo():
    ...
    for k, v in bar:
        ...
        weight = compute_weight(df_weight, total_days, method="MontoCarlo", starttime=start_time, endtime=end_time)
        weight_list.append(weight)
    # 保存权重
    filename = os.path.join(os.getcwd(), 'weights')
    if not os.path.exists(filename):
...
def compare_performance(method="Markowitz"):
    ...
    six_map, compare_list = get_six_month_map(x_matrix_total_hs300)
    ...
    for index, period in enumerate(compare_list):
        ...
        day_yield_compare_matrix = day_yield_compute(period_day_yield_matrix)
        ...
        print("\033[0;36;m 完成\033[0m \033[0;34;m{}--{}\033[0m  \033[0;36;m时间段比较，开始做图 \033[0m".format(start_time,
                                                                                                     end_time))
        plot_performance_compare(day_yield_compare_matrix, start_time, end_time, method)
        if index == 0:
...
if __name__ == '__main__':
    ...
    total_compare_yield_matrix = compare_performance(method=method)
    ...
    # 做一个总的投资组合和沪深300比较图
    plot_performance_compare(total_compare_yield_matrix, 20150105, 20191230, method)
